...
static void print_usage(int rank) {
    ...
    std::cout
        << "Usage:\n"
        << "  mpiexec -np <np> ./main --algo=ivf-mpi-omp --nlist=512 --nprobe=64 --test=2000 --threads=2 --k=10\n"
        << "\n"
        << "Options:\n"
        << "  --algo=ivf-mpi-omp      Algorithm label printed in results\n"
...
int main(int argc, char **argv) {
    ...
    if (cfg.partition != "block") {
        abort_all("this 3.2 implementation currently supports only --partition=block", rank);
    }
    omp_set_dynamic(0);
    ...
    if (rank == 0) {
        ...
        std::cout << "csv_header: run_id,algorithm,np,threads,nodes,ppn,total_cores,nlist,nprobe,k,test_queries,partition,cache_hit,recall,latency_us,bcast_us,search_avg_us,search_max_us,gather_us,merge_us,candidate_count_avg,work_min,work_avg,work_max,imbalance_ratio,batch_time_ms,qps\n";
        std::cout << "csv: "
                  << cfg.run_id << ","
                  << cfg.algo << ","
                  << world_size << ","
