...
static void print_usage(int rank) {
    ...
    std::cout
        << "Usage:\n"
        << "  mpiexec -np <np> ./main --algo=ivf-mpi --nlist=512 --nprobe=64 --test=2000 --k=10\n"
        << "\n"
        << "Options:\n"
        << "  --algo=ivf-mpi          Algorithm label printed in results\n"
        ...
        << "  --partition=block       Base partition strategy for this version\n"
        << "  --nodes=<nodes>         Optional record-only qsub nodes value\n"
        << "  --ppn=<ppn>             Optional record-only qsub ppn value\n"
        << "  --threads=1             Optional record-only threads per rank value\n"
...
static bool load_local_ivf_index(LocalIVFIndex &index,
    ...
    const size_t local_count = loaded.global_end - loaded.global_begin;
    if (loaded.list_offsets.size() != loaded.nlist + 1 ||
        loaded.ids.size() != local_count ||
        loaded.vectors.size() != local_count * loaded.vecdim ||
        loaded.list_offsets.empty() ||
        loaded.list_offsets.front() != 0 ||
        loaded.list_offsets.back() != local_count) {
        return false;
    }
...
int main(int argc, char **argv) {
    ...
    if (cfg.partition != "block") {
        abort_all("this 3.1 implementation currently supports only --partition=block", rank);
    }
    ...
    const std::string local_cache_path = local_ivf_cache_path(
        cfg.nlist, vecdim, base_number, world_size, rank, local_begin, local_end);

    const double local_build_start = MPI_Wtime();
    LocalIVFIndex local_index;
    const bool local_cache_hit = load_local_ivf_index(local_index,
                                                      local_cache_path,
                                                      cfg.nlist,
                                                      vecdim,
                                                      base_number,
    ...
    if (!local_cache_hit) {
        local_index = build_local_ivf_index(
            base, base_number, vecdim, centroids, cfg.nlist, rank, world_size);
    ...
    const unsigned long long local_work =
        static_cast<unsigned long long>(local_index.global_end - local_index.global_begin);
    ...
    for (size_t qi = 0; qi < test_number; ++qi) {
        ...
        if (rank == 0) {
            ...
            selected_lists = select_ivf_lists(centroids, cfg.nlist, vecdim, query_buffer.data(), cfg.nprobe);
        }
        ...
        ResultHeap local_result = search_local_ivf(
            local_index, query_buffer.data(), selected_lists, cfg.k, local_candidates_u64);
    ...
    if (rank == 0) {
        ...
        std::cout << "csv_header: algorithm,np,threads,nodes,ppn,nlist,nprobe,k,test_queries,partition,cache_hit,recall,latency_us,bcast_us,search_avg_us,search_max_us,gather_us,merge_us,candidate_count_avg,work_min,work_avg,work_max,imbalance_ratio,batch_time_ms,qps\n";
        std::cout << "csv: "
                  << cfg.algo << ","
                  << world_size << ","
                  << cfg.threads << ","
