...
echo "submit: nodes=${NODES_REQ} ppn=${PPN_REQ} np=${NP} threads=${THREADS} total_cores=${TOTAL_CORES} nlist=${NLIST} nprobe=${NPROBE} test=${TEST_QUERIES} k=${K} partition=${PARTITION} group=${GROUP}"
JOB_ID=$(qsub -l nodes=${NODES_REQ}:ppn=${PPN_REQ} -v NP=${NP},NLIST=${NLIST},NPROBE=${NPROBE},TEST_QUERIES=${TEST_QUERIES},K=${K},THREADS=${THREADS},NODES_REQ=${NODES_REQ},PPN_REQ=${PPN_REQ},ALGO=${ALGO},PARTITION=${PARTITION},GROUP=${GROUP} qsub_mpi.sh)
echo "${JOB_ID}"

if [ "${WAIT_FOR_RESULT:-1}" = "0" ]; then
    echo "submitted without waiting."
    echo "check later: qstat ${JOB_ID}"
    echo "show latest rows: tail -n 5 files/ivf_mpi_3_3_results.csv"
    echo "show all rows: cat files/ivf_mpi_3_3_results.csv"
    exit 0
fi
...
while qstat "${JOB_ID}" >/dev/null 2>&1; do
    ...
    if [ "${JOB_STATE}" = "Q" ] && [ "${MAX_QUEUE_WAIT}" -gt 0 ] && [ "${WAIT_SECONDS}" -ge "${MAX_QUEUE_WAIT}" ]; then
        ...
        echo "show latest rows after completion: tail -n 5 files/ivf_mpi_3_3_results.csv"
        echo "show all rows after completion: cat files/ivf_mpi_3_3_results.csv"
        exit 0
    fi
...
SUMMARY_FILE=files/ivf_mpi_3_3_results.csv
