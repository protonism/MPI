...
RUN_LOG=${RESULT_DIR}/ivf_mpi_3_3_${PBS_JOBID}.out
MASTER_RESULT_DIR=/home/${USER}/ann/files
MASTER_RESULT_CSV=${MASTER_RESULT_DIR}/ivf_mpi_3_3_results.csv
...
mkdir -p ${RESULT_DIR}
cd /home/${USER}
/usr/local/bin/mpiexec -np ${NP} -machinefile $PBS_NODEFILE /home/${USER}/main --algo=${ALGO} --nlist=${NLIST} --nprobe=${NPROBE} --test=${TEST_QUERIES} --k=${K} --threads=${THREADS} --nodes=${NODES_REQ} --ppn=${PPN_REQ} --partition=${PARTITION} --group=${GROUP} --run-id=${RUN_ID} | tee ${RUN_LOG}

HEADER=$(grep '^csv_header: ' ${RUN_LOG} | tail -n 1 | sed 's/^csv_header: //')
CSV_LINE=$(grep '^csv: ' ${RUN_LOG} | tail -n 1 | sed 's/^csv: //')

if [ -n "${CSV_LINE}" ]; then
    ssh master_ubss1 "mkdir -p ${MASTER_RESULT_DIR}; lock=${MASTER_RESULT_DIR}/.ivf_mpi_3_3_results.lock; while ! mkdir \"\$lock\" 2>/dev/null; do sleep 1; done; trap 'rmdir \"\$lock\"' EXIT; if [ ! -s ${MASTER_RESULT_CSV} ] && [ -n '${HEADER}' ]; then printf '%s\n' '${HEADER}' >> ${MASTER_RESULT_CSV}; fi; printf '%s\n' '${CSV_LINE}' >> ${MASTER_RESULT_CSV}" 2>&1
