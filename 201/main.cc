...
static void print_usage(int rank) {
    ...
    std::cout
        << "Usage:\n"
        << "  mpiexec -np <np> ./main --algo=hnsw-shard-mpi --ef=100 --M=16 --efc=150 --test=2000 --partition=block-id\n"
        << "\n"
        << "Options:\n"
        << "  --algo=hnsw-shard-mpi  Algorithm label printed in results\n"
        << "  --data=/anndata/       Dataset directory\n"
        << "  --ef=100               HNSW efSearch\n"
        << "  --M=16                 HNSW M\n"
        << "  --efc=150              HNSW efConstruction\n"
        << "  --test=2000            Max query count\n"
        << "  --k=10                 Recall@k and returned top-k\n"
        << "  --partition=block-id   block-id, cyclic, or random-hash\n"
        << "  --group=manual         Experiment group label recorded in CSV\n"
        << "  --threads=1            Record-only threads field for compatibility\n"
        << "  --nodes=<nodes>        Record-only qsub nodes value\n"
        << "  --ppn=<ppn>            Record-only qsub ppn value\n"
        << "  --run-id=manual        Unique run id recorded in CSV\n"
        << "  --rebuild              Rebuild local HNSW caches\n";
}
...
static Config parse_args(int argc, char **argv, int rank) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        } else if (starts_with(arg, "--ef=")) {
            cfg.ef_search = parse_int_value(value_after_equal(arg), cfg.ef_search);
        } else if (starts_with(arg, "--M=")) {
            cfg.M = parse_int_value(value_after_equal(arg), cfg.M);
        } else if (starts_with(arg, "--efc=")) {
            cfg.ef_construction = parse_int_value(value_after_equal(arg), cfg.ef_construction);
        } else if (arg == "--rebuild") {
            cfg.rebuild = true;
        }
    }
    ...
    } else if (cfg.partition == "round-robin") {
        cfg.partition = "cyclic";
    } else if (cfg.partition == "random" || cfg.partition == "hash") {
        cfg.partition = "random-hash";
    ...
    cfg.ef_search = std::max<int>(static_cast<int>(cfg.k), cfg.ef_search);
    cfg.M = std::max(1, cfg.M);
    cfg.ef_construction = std::max(1, cfg.ef_construction);
...
static std::string local_hnsw_cache_path(size_t vecdim,
                                         size_t base_number,
                                         int world_size,
                                         int rank,
                                         const Config &cfg) {
    std::ostringstream oss;
    oss << "files/hnsw_shard_d" << vecdim
        << "_n" << base_number
        << "_np" << world_size
        << "_rank" << rank
        << "_part" << cache_token(cfg.partition)
        << "_M" << cfg.M
        << "_efc" << cfg.ef_construction
        << ".index";
    return oss.str();
}

static void partition_block(size_t n, int rank, int world_size, size_t &begin, size_t &end) {
    begin = n * static_cast<size_t>(rank) / static_cast<size_t>(world_size);
    end = n * static_cast<size_t>(rank + 1) / static_cast<size_t>(world_size);
}

static bool owns_id(size_t id, size_t base_number, int rank, int world_size, const std::string &partition) {
    ...
    if (partition == "cyclic") {
        return static_cast<int>(id % static_cast<size_t>(world_size)) == rank;
    }
    const uint64_t owner = splitmix64(static_cast<uint64_t>(id)) % static_cast<uint64_t>(world_size);
    return static_cast<int>(owner) == rank;
}

static std::vector<uint32_t> assigned_ids(size_t base_number,
                                          int rank,
                                          int world_size,
    ...
    ids.reserve(base_number / static_cast<size_t>(world_size) + 1);
    for (size_t id = 0; id < base_number; ++id) {
        if (owns_id(id, base_number, rank, world_size, partition)) {
            ids.push_back(static_cast<uint32_t>(id));
        }
    }
    return ids;
...
static inline void push_topk(ResultHeap &heap, float dist, uint32_t id, size_t k) {
    if (heap.size() < k) {
...
static ResultHeap hnsw_search(HnswIndex &index, const float *query, size_t k) {
    ResultHeap result;
    std::priority_queue<std::pair<float, hnswlib::labeltype> > raw = index.searchKnn(query, k);
    while (!raw.empty()) {
        const std::pair<float, hnswlib::labeltype> item = raw.top();
        raw.pop();
        push_topk(result, item.first, static_cast<uint32_t>(item.second), k);
    }
    return result;
}
...
static HnswIndex *load_or_build_local_index(hnswlib::InnerProductSpace &space,
                                            const float *base,
                                            size_t base_number,
                                            size_t vecdim,
                                            const Config &cfg,
                                            ...
                                            bool &cache_hit) {
    cache_hit = false;
    if (!cfg.rebuild && file_exists(path)) {
        std::cerr << "rank " << rank << " load local HNSW cache " << path << "\n";
        HnswIndex *index = new HnswIndex(&space, path, false);
        index->setEf(static_cast<size_t>(cfg.ef_search));
        cache_hit = true;
        return index;
    }

    if (ids.empty()) {
        abort_all("rank has no assigned ids; reduce np or change partition", rank);
    }

    std::cerr << "rank " << rank << " build local HNSW: points=" << ids.size()
              << " M=" << cfg.M << " efConstruction=" << cfg.ef_construction << "\n";
    HnswIndex *index = new HnswIndex(&space, ids.size(), cfg.M, cfg.ef_construction);
    for (size_t i = 0; i < ids.size(); ++i) {
        const uint32_t id = ids[i];
        if (static_cast<size_t>(id) >= base_number) {
            abort_all("assigned id out of range", rank);
        }
    ...
    try {
        ...
        std::cerr << "rank " << rank << " save local HNSW cache " << path << "\n";
    } catch (const std::exception &e) {
        std::cerr << "rank " << rank << " warning: failed to save local HNSW cache: "
                  << e.what() << "\n";
    }
    (void)world_size;
    return index;
...
int main(int argc, char **argv) {
    ...
    const std::vector<uint32_t> local_ids =
        assigned_ids(base_number, rank, world_size, cfg.partition);
    const std::string index_path =
        local_hnsw_cache_path(vecdim, base_number, world_size, rank, cfg);

    hnswlib::InnerProductSpace ipspace(vecdim);
    const double local_build_start = MPI_Wtime();
    bool local_cache_hit = false;
    HnswIndex *local_index = load_or_build_local_index(
        ipspace, base, base_number, vecdim, cfg, rank, world_size, local_ids, index_path, local_cache_hit);
    const double local_build_ms = (MPI_Wtime() - local_build_start) * 1000.0;

    const unsigned long long local_work = static_cast<unsigned long long>(local_ids.size());
    std::vector<unsigned long long> all_work;
    std::vector<double> all_build_ms;
    std::vector<int> all_cache_hit;
    if (rank == 0) {
        all_work.assign(world_size, 0);
        all_build_ms.assign(world_size, 0.0);
        all_cache_hit.assign(world_size, 0);
    }
    ...
    MPI_Gather(&local_cache_hit_int, 1, MPI_INT,
               rank == 0 ? all_cache_hit.data() : NULL, 1, MPI_INT,
               0, MPI_COMM_WORLD);

    MPI_Barrier(MPI_COMM_WORLD);
    if (rank == 0) {
        std::cerr << "start HNSW shard MPI search: np=" << world_size
    ...
    std::vector<uint32_t> local_result_ids;
    ...
    for (size_t qi = 0; qi < test_number; ++qi) {
        ...
        MPI_Bcast(query_buffer.data(),
                  ...
                  0,
                  MPI_COMM_WORLD);
        const double bcast_us = rank == 0 ? (MPI_Wtime() - bcast_start) * 1000000.0 : 0.0;

        const double search_start = MPI_Wtime();
        ResultHeap local_result = hnsw_search(*local_index, query_buffer.data(), cfg.k);
        const double local_search_us = (MPI_Wtime() - search_start) * 1000000.0;
        fill_topk_buffers(local_result, local_distances, local_result_ids, cfg.k);
        ...
        MPI_Gather(local_distances.data(), mpi_count(cfg.k, "local distances", rank), MPI_FLOAT,
                   ...
                   0,
                   MPI_COMM_WORLD);
        MPI_Gather(local_result_ids.data(), mpi_count(cfg.k, "local ids", rank), MPI_UNSIGNED,
        ...
        if (rank == 0) {
            ...
            metrics.candidate_count_sum += static_cast<double>(world_size * static_cast<int>(cfg.k));
        }
    }
    ...
    if (rank == 0) {
        ...
        for (int r = 0; r < world_size; ++r) {
            ...
            local_cache_hits += all_cache_hit[r];
        }
        ...
        std::cout << "M: " << cfg.M << "\n";
        std::cout << "efConstruction: " << cfg.ef_construction << "\n";
        std::cout << "efSearch: " << cfg.ef_search << "\n";
        std::cout << "k: " << cfg.k << "\n";
        ...
        std::cout << "csv_header: run_id,group,algorithm,np,threads,nodes,ppn,total_cores,M,efConstruction,efSearch,k,test_queries,partition,cache_hit,recall,latency_us,bcast_us,search_avg_us,search_max_us,gather_us,merge_us,candidate_count_avg,work_min,work_avg,work_max,imbalance_ratio,build_avg_ms,build_max_ms,batch_time_ms,qps\n";
        std::cout << "csv: "
                  << cfg.run_id << ","
                  << cfg.group << ","
                  << cfg.algo << ","
                  ...
                  << test_number << ","
                  << cfg.partition << ","
                  << (local_cache_hits == world_size ? 1 : 0) << ","
                  << avg_recall << ","
                  << avg_latency << ","
                  << avg_bcast << ","
                  << avg_search_avg << ","
